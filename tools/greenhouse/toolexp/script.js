        ...
        files.forEach((file) => {
            ...
            reader.onload = (event) => {
                ...
                img.onload = () => {
                    ...
                    // 自動觸發載入第一張圖片
                    if (!state.image) itemWrapper.click();
                };
...
// --- 5. 影像處理與抖動演算法 ---
    ...
    function updateAggregateHUD(origNum, estNum, darkPixels, totalPixels) {
        ...
        if(hudOrig) hudOrig.innerText = `${origNum.toFixed(2)}KB`;
        if(hudSize) hudSize.innerText = `${estNum.toFixed(2)}KB`;
        
        if (totalPixels > 0) {
            const powerSaved = Math.round((darkPixels / totalPixels) * 100);
            if(hudPower) hudPower.innerText = `${powerSaved}%`;
        } else {
            if(hudPower) hudPower.innerText = `0%`;
        }
        ...
        if (origNum && origNum > 0) {
            ...
            if(hudSaved) hudSaved.innerText = `0.0%`;
        }
    }
